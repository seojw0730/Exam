...
//	전달받은 키워드가 포함된 도서가 여러 개가 존재할 수 있으니 검색된 도서를
//	담아줄 Book 객체 배열을 새로이 생성하고 for문을 통해 bList 안의 도서들과
//	전달받은 키워드를 비교하여 포함하고
//	있는 경우 새로운 배열에 차곡차곡
//	담기
//	→ 그 배열 주소 값 리턴
...
//	→ result 리턴	
	public int rentBook(int index) {
		int result = 0;
//		전달 받은 index의 bList객체가 만화책을 참조?
		if (bList[index] instanceof AniBook) {
			AniBook ab = (AniBook) bList[index];
			if (ab.getAccessAge() > mem.getAge()) {
				result = 1;
			}
//		전달 받은 index의 bList객체가 요리책을 참조?
		} else if (bList[index] instanceof CookBook) {
			CookBook cb = (CookBook) bList[index];
			if (cb.isCoupon()) {
//				int numCoupon = mem.getCouponCount();
//				numCoupon++;
//				mem.setCouponCount(++numCoupon);
